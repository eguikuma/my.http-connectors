...
export const failify = async (
  ...
  return {
    success: false,
    status: hasReason ? thrown.status : StatusCodes.INTERNAL_SERVER_ERROR,
    message: hasReason ? thrown.message : getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR),
    ...(hasReason && has<Failed>('body')(thrown) ? { body: thrown.body } : {}),
  }
}
